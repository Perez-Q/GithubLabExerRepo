...
def sign_up():
    if request.method == 'POST':
        ...
        about_me = "The user has not set up his/her about me"
        user = User.query.filter_by(email=email).first()
        if user:
            ...
            new_user = User(email=email, first_name=first_name, password=generate_password_hash(password1, method='pbkdf2:sha256'),about_me=about_me)
